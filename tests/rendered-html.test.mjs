...
async function render(pathname = "/") {
  const workerUrl = new URL("../.artifacts/dist/server/index.js", import.meta.url);
...
test("declares each supported elementary-stream format", async () => {
  ...
  assert.doesNotMatch(analyzer, /copy\.(?:dimensions|frameRate)/);
  assert.doesNotMatch(analyzer, /analysis\.duration/);
});
